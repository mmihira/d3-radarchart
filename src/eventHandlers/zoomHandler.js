...
function zoomHandler (zoom) {
  ...
  return function () {
    ...
    if (browserVendor.isFirefox) {
      const minZoom = k > zoomProps.scaleExtent.minZoom ? k * 0.1 : zoomProps.scaleExtent.minZoom;
      const maxZoom = k < zoomProps.scaleExtent.maxZoom ? k * 1.1 : zoomProps.scaleExtent.maxZoom;
    ...
    if (d3.event.transform.k === 1) {
      self.state.selectors.selectRootG()
