...
class Legend {
  ...
  render () {
    ...
    this.selectors.selectLegendG()
      .selectAll('legend-rect')
      .data(areaDatums)
      .enter()
      .append('rect')
      .attr('class', (d) => this.selectors.legendRectNameForSeries(d.props.seriesId))
      .attr('x', width - (legendW * (1 + legendOpts.legendWOverlap)))
      ...
      .each(function (d) { setAreaLegendRect(d.props.seriesId, this); });

    // Create text next to squares
    this.selectors.selectLegendG().selectAll('text')
      .data(areaDatums)
      .enter()
      .append('text')
      .text('')
      .each(function (d, z) {
        var lines = d.props.legendLabelLines;
        for (let i = 0; i < lines.length; i++) {
          d3.select(this)
            .append('tspan')
            .attr('x', width - (legendW * (1 + legendOpts.legendWOverlap)) * legendOpts.textOffsetP)
            .attr('y', d => z * legendDims.iconSpacing + legendOpts.textYOffset)
            .attr('dy', d => legendOpts.labelTextLineSpacing(width) * i)
            .text(d => d.props.legendLabelLines[i])
            .style('font-size', () => {
              if (legendOpts.scaleTextWithSize) {
            ...
            .each(function (d) { setAreaLegendLabelLines(d.props.seriesId, this); });
        }
      });

    this.selectors.selectLegendG().selectAll('legend-rect-overlays')
      .data(areaDatums)
      .enter()
      .append('rect')
      .attr('class', (d) => this.selectors.legendRectOverlayClassName(d.props.seriesId))
      .attr('x', width - (legendW * (1 + legendOpts.legendWOverlap)))
      .attr('y', (d, i) => i * legendDims.iconSpacing)
      ...
      .each(function (d) { setAreaLegendRectOverlay(d.props.seriesId, this); });
  }

  removeLegend (seriesIds) {
    seriesIds.forEach(seriesId => {
      this.selectors.selectLegendOverlaysForSeries(seriesId)
        .on('mouseover', null)
        .on('mouseout', null);
    });
