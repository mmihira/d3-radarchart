...
test('__buildAxis', () => {
  ...
  const propsToTest = omit(state.components.axis.con_1.props, toOmit);

  expect(propsToTest).toEqual({
    angleFromNorth: -91.50778367139691,
    axisId: 'con_1',
    axisLength: 190,
  ...
  const projectValueOnAxis =
    state.components.axis.con_1.props.projectValueOnAxis;
  ...
  expect(axisTitleScale(100)).toEqual(
    stateDefaultOpts.axis.axisScaleProps.minTitleSize
  );
  expect(axisTitleScale(1200)).toEqual(
    stateDefaultOpts.axis.axisScaleProps.maxTitleSize
  );
